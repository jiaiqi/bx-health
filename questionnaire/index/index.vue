<template>
  <view
    ...
    :style="{
      ...
      '--primary-color': '#F34A11',
      '--primary-color-hover': '#D91616',
  ...
  >
    ...
    >
      <button
        ...
        type=""
        @click="submitForm()"
      > <text v-if="formData && formData.submit_label">{{ formData.submit_label }}</text><text v-else>提交</text></button>
    </view>
    <view
      class="button-box"
...
export default {
  ...
  computed: {
    ...
    customTitleImage() {
      if (this.formData.activity_image) {
        return this.getImagePath(this.formData.activity_image, true)
      }
    },
    customTitleBg() {
      if (this.formData.activity_image) {
        return `--title-bg: url(${this.customTitleImage});`
      }
    }
  },
  watch: {
  ...
  onLoad(option) {
    if (option.submitInfo) {
      this.submitInfo = JSON.parse(decodeURIComponent(option.submitInfo));
    }
    if (option.inFrame) {
      this.inFrame = true
    }
