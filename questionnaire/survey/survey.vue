...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toHistory() {
        uni.navigateTo({
          url: '/questionnaire/survey/history?activity_no=' + this.activity_no + '&title=' + this
            .formData.title
        });
      },
      seeScore() {
      ...
      async seeReport() {
        // 查看评估结果
        uni.navigateTo({
          url: `/questionnaire/survey/result?activity_no=${this.activity_no}&fill_batch_no=${this.fill_batch_no}`
        });
      },
      toFeedback() {
        // 跳转到意见反馈问卷
        uni.redirectTo({
          url: '/questionnaire/survey/survey?formType=form&activity_no=20200309125000000100&status=进行中'
        });
      },
      saveValue(e) {
        let self = this;
                  ...
                  if (self.params.to && self.params.fill_batch_no) {
                    ...
                    if (self.params.fill_batch_no) {
                      ...
                      if (self.formData.info_collect_type === '评估') {
                        const url =
                          `/questionnaire/survey/result?activity_no=${self.activity_no}&fill_batch_no=${self.fill_batch_no}`
                        uni.redirectTo({
                          url
                        })
                        return
    ...
    onShareAppMessage() {
      let path =
        `/questionnaire/survey/survey?formType=form&activity_no=${this.activity_no}&status=进行中`;
...
<style lang="scss" scoped>
  ...
  .content {
    ...
    .desc {
      width: 100%;
      background: linear-gradient(88.92deg, rgba(92, 224, 168, 0.41) 1.07%, rgba(195, 228, 214, 0.22) 99.94%);
      ;
    }
