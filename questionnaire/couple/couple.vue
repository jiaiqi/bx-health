			...
			<view class="couple-cen">
				<view @click="chooseItem(item)" v-for="(item,index) in coupleData" :key="index" class="couple-cen-item">
					<view class="couple-cen-item-t-t">
						<text v-if="item.grade">已填写</text>
					</view>
					<view v-if="item.name !== '其它'" class="couple-cen-item-t" :class="!item.grade?'':item.grade>0&&item.grade<=50?'risk':'normal'">					
						<text v-if="item.grade">{{item.grade}}</text>
