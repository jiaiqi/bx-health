...
<script>
	import popupLayer from '@/questionnaire/components/sl-filter/popup-layer.vue';
	import slFilterView from '@/questionnaire/components/sl-filter/filter-view.vue';
	export default {
		components: {
			popupLayer,
			slFilterView
			...
			filterResult(obj) {
				...
				if (this.independence) {
					if (!this.menuList[this.selectedIndex].isMutiple || this.menuList[this.selectedIndex].isSort) {
						...
						if (this.menuList[this.selectedIndex].reflexTitle&&tempTitle) {
				...
				let dataItemSelect = [{'result':val},{'title':obj.present}]
				this.$refs.popupRef.close()
				if (obj.isReset) {
				} else{
					this.$emit("result", dataItemSelect)
				}
			},
			close() {
				for (let i = 0; i < this.statusList.length; i++) {
