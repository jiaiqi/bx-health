<template>
  <view class="form" :class="{'is-pc':isPC}" :style="{
    ...
    <scroll-view scroll-y="true">
      <view>
        <a-form v-if="colsV2Data && isArray(fields)" :fields="fields" :pageType="srvType" :formType="use_type" :ui="ui"
...
<script>
  ...
  export default {
    ...
    computed: {
      setService() {
        return this.formService || this.serviceName || this.service
      },
      setFormType() {
        return this.formType || this.type
      },
      setApp() {
        return this.formApp || this.appName
      },
      isPC() {
        // #ifdef H5
          ...
          if (e.foreign_key.more_config && e.foreign_key.more_config.targetType) {
            ...
            if (targetType === 'list') {
              ...
              if (e.childData && e.childData.data && e.childData.data.length > 0) {
                ...
                let url =
                  `/publicPages/newForm/newForm?type=update&serviceName=${e.service_name}&fieldsCond=${encodeURIComponent(JSON.stringify(fieldsCond))}`;
                uni.navigateTo({
                  url: url
                });
              } else {
    ...
    created() {
      if (this.formService && this.formType) {
        this.appName = this.formApp
        this.serviceName = this.formService
