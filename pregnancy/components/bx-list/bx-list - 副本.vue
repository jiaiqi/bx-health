...
export default {
  ...
  computed: {
    ...
    //   } else if (this.listConfig && this.listConfig.hasOwnProperty('srv_cols')) {
    //     let rowButton = this.listConfig .rowButton;
    //     if (rowButton) {
  ...
  async mounted() {
    if (this.serviceName && this.listType === 'list') {
      ...
      for(let i=0;i<this.tabList.length;i++){
        ...
        await this.getListData([], item.proc_data_type)
      }
        this.tabList.forEach(item => {
          this.getListData([], item.proc_data_type)
          // .then(data => {
