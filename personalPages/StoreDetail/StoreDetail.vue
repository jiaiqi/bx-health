			...
			<view class="content">
				<view class="professor-box">
					<view class="professor-item" v-for="custom in customerList" :data-item="custom" @click="toNav(custom, 'custom', $event)">
						...
						<view class="doc-info">
							<view class="top">
								<text class="doc-name">{{ custom.person_name || custom.nick_name ||'' }}</text>
							</view>
						</view>
					</view>
				</view>
	...
	export default {
		...
		computed: {
			staffList() {
				if (Array.isArray(this.storeUserList)) {
					return this.storeUserList.filter(item => item.user_role && (item.user_role.indexOf('用户') === -1 || item.user_role.indexOf(
						'用户,') !== -1));
				} else {
					return [];
				}
		...
		methods: {
			...
			radioChange(e) {
				if (e === '查看员工信息') {
					uni.navigateTo({
						url: '/personalPages/DoctorDetail/DoctorDetail?doctor_no=' + this.curStaff.person_no + '&store_no=' + this.curStaff
							.store_no
					});
				} else if (e === '员工管理') {
					let fieldsCond = [{
					...
					uni.navigateTo({
						url: '/publicPages/newForm/newForm?serviceName=srvhealth_store_user_update&type=update&fieldsCond=' +
