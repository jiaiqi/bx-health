<template>
	<view class="form-page">
		<view class="drug-select-box" v-if="fields && fields.length > 0">
			<!-- 药品选择 -->
			<view class="title" v-if="drugList && drugList.length > 0">{{ addType === 'sport' ? '运动项目清单' : '药物清单' }}
			</view>
			<view class="drug-list" v-if="drugList && drugList.length > 0">
				<view class="drug-item" v-for="item in drugList" :class="{ active: item.checked }"
					@click="changeDrugChecked(item)">
		...
		<a-form ref="bxForm" v-if="colsV2Data && isArray(fields)" :fields="fields" :pageType="type"
			:service="serviceName" :formType="type"
			:moreConfig="colsV2Data && colsV2Data.more_config ? colsV2Data.more_config : null"></a-form>
		<view class="button-box" v-if="formType === 'detail' && isArray(fields) && fields.length > 0"><button
				class="cu-btn bg-red button" @click="deleteItem">删除</button></view>
			...
			async getSportItemInfo(list) {
				...
				let req = {
					...
					condition: [{
						colName: 'sport_no',
						ruleType: 'in',
						value: list.map(item => item.sport_no).toString()
					}]
				};
				if (this.formType === 'detail') {
					req.condition = [{
						colName: 'plan_no',
						ruleType: 'eq',
						value: this.defaultVal.ds_no
					}, {
						colName: 'dsr_no',
						ruleType: 'eq',
						value: this.defaultVal.dsr_no
					}];
				}
				...
				if (Array.isArray(res.data.data) && res.data.data.length > 0) {
					list = list.map(item => {
						if (res.data.data.find(d => d.sport_no === item.sport_no)) {
							...
							item.unit_amount = res.data.data.find(d => d.sport_no === item.sport_no)[
								'unit_amount'];
							item.unit_energy = res.data.data.find(d => d.sport_no === item.sport_no)[
								'unit_energy'];
							item.checked = false;
							if (this.formType === 'detail') {
								item.checked = true;
