<template>
	<view class="history-wrap">
		...
		<view class="history-chart" v-if="pageType==='step'">
			<uniEcCharts class="uni-ec-charts" id="uni-ec-canvas" v-if="currentChart === 'stepChart'" :ec="stepEcData">
			</uniEcCharts>
		</view>
		...
		<view class="history-box" v-if="pageType">
				...
				<button class="cu-btn bg-cyan symptom-add-button shadow-blur" v-if="pageType === 'symptom'"
					@click="toPages('symptom')">添加</button>
			</view>
			<view class="history-content">
				<dietList :chatChoseTime="chatChoseTime" :key="pageType" @changePageType="changePageType"
					:pageType="pageType" v-if="pageType === 'diet' || pageType === 'sport'"></dietList>
				<view class="other-record"
					v-if="pageType === 'sleep' || pageType === 'bp'  || pageType === 'bg' || pageType === 'weight'"
					:class="{
					...
					<view class="record-data" v-if="pageType !== 'weight'">
						<view class="last-data"
							v-if="isArray(historyRecord) && historyRecord.length > 0 && (pageType === 'bp'||pageType === 'bg')">
							<text class="digital ">
								<text v-if="pageType === 'bp'">
									<text :class="{
								...
								<text
									v-if="pageType === 'bg'">{{getFixedNum(historyRecord[0].blood_glucose_val)}}</text>
							</text>
							<text class=" unit" v-if="pageType === 'bp'">毫米汞柱</text>
							<text class="unit" v-if="pageType === 'bg'">毫摩尔每升</text>
						</view>
						<view class="last-data"
							v-if="isArray(historyRecord) && historyRecord.length > 0 && pageType === 'sleep'">
							<text class="digital"
								v-if="historyRecord[0].sleep_time">{{ historyRecord[0].sleep_time.slice(0, 5) }}</text>
						...
						<button class="nav-button" @click="toPages(pageType)"
							v-if="!this.customer_no&&pageType!=='bg'">记录数据</button>
					</view>
					...
					<view class="data-chart" v-if="pageType === 'weight' || pageType === 'bp'|| pageType === 'bg'">
						...
						<uniEcCharts class="uni-ec-charts" id="uni-ec-charts" :ec="GluChartOption"
							v-if="pageType === 'bg'"></uniEcCharts>
					</view>
					<view class="history-record">
						...
						<view class="list-box cu-list" v-if="historyRecord && historyRecord.length > 0">
							<view class="cu-item list-item" :class="modalName == 'move-box-' + index ? 'move-cur' : ''"
								...
								<image src="../static/icon/xueya.png" mode="" class="icon"
									v-if="(pageType === 'bp'||pageType === 'bg')">
								</image>
								...
								<view class="content">
									<view class="item">
										...
										<text class="digital" :class="{
										                  'text-green': item.blood_glucose_val < 6.1,
										                  'text-yellow': item.blood_glucose_val >= 6.1 && item.blood_glucose_val < 9.4,
										                  'text-red': item.blood_glucose_val >= 9.4,
										                }" v-if="pageType === 'bg'">{{ item.blood_glucose_val ? getFixedNum(item.blood_glucose_val) : '-' }}</text>
										<text class="digital"
											v-if="pageType === 'weight'">{{ item.weight ? getFixedNum(item.weight) : '-' }}</text>
										<text class="digital"
											v-if="pageType === 'sleep'">{{ item.sleep_time ? item.sleep_time.slice(0, 5) : '-' }}</text>
									</view>
									...
									<view class="unit">
										...
										<text v-if="pageType === 'bg'">mmol/L</text>
		...
		<view class="cu-modal bottom-modal" :class="{ show: showSymptom }" @click="showSymptom = false">
			<view class="cu-dialog" @tap.stop="">
				<view class="symptom-selector" :class="{ 'low-height': showSymptomDateSelector }" v-if="showSymptom">
					...
					<view class="button-box">
						<button type="primary" class="cu-btn next-btn bg-cyan round"
							v-if="currentSymptom && currentSymptom.node_type === '症状' && !showSymptomDateSelector"
							@click="symptomNext('next')">
							下一步
						</button>
						<button class="cu-btn next-btn bg-gray round"
							v-if="currentSymptom && currentSymptom.node_type === '症状' && showSymptomDateSelector"
	...
	export default {
		...
		methods: {
			buildBGOption(data) {
				...
				const beforeEatLine = data.filter(item => item.glucose_time === '空腹').map(item => item.blood_glucose_val)
				const afterEatLine = data.filter(item => item.glucose_time.indexOf('餐后') !== -1).map(item => item
					.blood_glucose_val)
				const xAxisData = data.map(item => dayjs(item.measure_time).format('MM-DD'))
				let max = data.map(item => item.blood_glucose_val).sort((a, b) => b - a)[0] + 0.5
				let min = data.map(item => item.blood_glucose_val).sort((a, b) => a - b)[0] - 0.5
			...
			showCanvas(type) {
				...
				switch (type) {
					...
					case 'bg':
					case 'BG':
						this.getChartData('bg').then(_ => {
							this.bpEcData = this.buildEcData(this.BPChartData, 'mmHg', '血糖')
						}) // 血糖
						this.currentType = '血糖'
						break
					case 'sleep':
			...
			async getChartData(type) {
				let serviceObj = {
					...
					sleep: 'srvhealth_sleep_record_select', // 睡眠
				...
				if (res.data.state === 'SUCCESS' && Array.isArray(res.data.data) && res.data.data.length > 0) {
					...
					if (type === 'weight') {
						...
						this.buildBGOption(res.data.data)
					}
