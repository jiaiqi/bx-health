<template>
	<view class="relation-graph" :style="{
		...
		<view class="detail-desc" v-if="nodeDetail">
			...
			<view class="node-img">
				<image class="image" :src="item" :key="item" mode="aspectFit" v-for="item in nodeImageList"></image>
			</view>
	...
	export default {
		...
		data() {
			return {
				...
				emptyText: '数据加载中...',
				currentNodes: 'KN202011211458080001', //健康
		...
		methods: {
			getNodeImagePath(nodetail) {
				if (this.nodeDetail.node_image) {
					...
					let req = {
						...
						condition: [{
							colName: 'file_no',
							value: this.nodeDetail.node_image,
							ruleType: 'eq'
						}, {
							colName: 'is_delete',
							value: '1',
			...
			async geteChartsData() {
				...
				let req = {
					...
					relation_condition: {
						relation: 'OR',
						data: [{
							colName: 'source_node_no',
							ruleType: 'eq',
							value: this.currentNodeNo
						}, {
							colName: 'target_node_no',
							ruleType: 'eq',
							value: this.currentNodeNo
						}]
					},
				...
				nodeData.forEach(node => {
					nodes.push({
						// #ifdef H5
						symbol: node && node.node_icon ? `image://${node.node_icon}` : node.kn_no === this.currentNodeNo ? 'diamond' :
							'circle', //中间的节点
						// #endif
						// #ifdef MP-WEIXIN
				...
				let links = data.map(item => {
					return {
						...
						source: item.source_node_no,
						sourceName: item.source_name,
						target: item.target_node_no,
						targetName: item.target_name,
						source_node_no: item.source_node_no,
						type: 'path',
						target_node_no: item.target_node_no,
						lineStyle: {
							color: item.source_node_no === self.currentNodes ? '#10a050' : '#006acc'
				...
				let option = {
					...
					series: [{
						...
						force: {
							...
							repulsion: nodes.length <= 2 ?
								900 : nodes.length < 5 && nodes.length > 2 ?
								800 : nodes.length >= 5 && nodes.length < 10 ?
								700 : nodes.length > 15 && nodes.length <= 20 ?
								600 : 500, // 节点之间的斥力因子,值越大斥力越大
							edgeLength: nodes.length <= 2 ?
								150 : nodes.length < 5 && nodes.length > 2 ?
								120 : nodes.length >= 5 && nodes.length < 7 ?
								100 : nodes.length >= 7 && nodes.length < 10 ?
								70 : nodes.length >= 10 && nodes.length < 13 ?
								70 : nodes.length >= 10 && nodes.length < 13 ?
								60 : nodes.length >= 13 && nodes.length <= 15 ?
								60 : nodes.length > 15 && nodes.length <= 18 ?
								30 : 20 //边的两个节点之间的距离，这个距离也会受 repulsion影响，支持设置成数组表达边长的范围，此时不同大小的值会线性映射到不同的长度。值越小则长度越长
						},
						data: nodes,
						links: links
					}]
							...
							switch (cur.link_type) {
								case '外部页面':
									...
									if (cur.external_page_link_url && cur.external_page_link_url.indexOf('https://view.inews.qq.com/a/') !== -1) {
										cur.external_page_link_url = cur.external_page_link_url.replace('https://view.inews.qq.com/a/',
				...
				if (res.data.state === 'SUCCESS' && Array.isArray(res.data.data) && res.data.data.length > 0) {
					...
					if (nodeDetail.node_desc) {
						if (nodeDetail.node_desc.indexOf('<img') !== -1) {
							nodeDetail.node_desc = nodeDetail.node_desc.replace(/<img/,
								'<img style="width:90%;height:90%;margin:0;padding:0;text-indent:0;"');
						}
					}
					...
					if (nodeDetail) {
						...
						this.currentNodes = this.nodeDetail.kn_no;
						this.getNodesLink();
						this.changeLinkPath({
			...
			async clickCharts(e) {
				if (e.data && e.data.nodeNo) {
					this.currentNodes = e.data.nodeNo;
					// this.currentNodes = e.name;
		...
		onShareAppMessage() {
			...
			if (this.userInfo && this.userInfo.userno && this.nodeDetail && this.nodeDetail.kn_no) {
				path =
					`/archivesPages/RelationGraph/RelationGraph?currentNodeNo=${this.nodeDetail.kn_no}&from=share&invite_user_no=${this.loginUserInfo.userno}`;
