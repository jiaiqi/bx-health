<template>
	<view class="handler-bar">
		<view class="cook-type-box" v-if="dietInfo&& dietInfo.diret_type!=='mixed_food'">
			...
			<view class="cook-type" :class="{ 'current-cook-type': item.value === cook_method }"
				@click="changeCookMethod(true, item.value)" v-for="item in defaultCookTypes" :key="item.value">
				{{ item.value }}
			</view>
		...
		<view class="dinner-time unit-box" v-if="dietInfo.food_no || dietInfo.diet_record_no">
			...
			<view class="unit-item" :class="{ 'active-unit': dining_type === u.value }" v-for="(u, index) in dinnerTime"
				:key="index" @click="changeDiningType(u, index)">
				{{ u.value }}
			</view>
		</view>
		<view class="dinner-time unit-box">
			<view class="title">单位:</view>
			<view class="unit-item" :class="{ 'active-unit': dietInfo.unit === u.unit }" v-for="(u, index) in unitList"
				:key="index" @click="checkUnit(u, index)">{{ getUnit(u) }}</view>
		</view>
		<view class="amount">
			<view class="title">数量:</view>
			<view class="number-box">
				<text @click="changeAmount('minus', 0.1)" class="symbol"
					:class="{ disabled: dietInfo.amount - 0.1 <= 0 }">-0.1</text>
				<text @click="changeAmount('minus')" class="symbol"
					:class="{ disabled: dietInfo.amount - 1 <= 0 }">-1</text>
				<!-- <input class="input" v-model.number="dietInfo.amount" type="digit" /> -->
				<view class="input">
					{{dietInfo.amount||''}}
				</view>
			...
			<view class="cu-dialog" @tap.stop="">
				...
				<view class="cooktype-wrap" v-if="dietInfo">
					<bx-radio-group v-model="cook_method" mode="button">
						<bx-radio v-for="item in cookTypes" :key="item.value" :name="item.value">{{ item.label }}
						</bx-radio>
					</bx-radio-group>
			...
			async getFoodUnit() {
				...
				let req = {
					...
					condition: [{
						colName: 'food_no',
						ruleType: 'eq',
						value: this.dietInfo.diet_contents_no ? this.dietInfo.diet_contents_no : this
							.dietInfo.food_no
					}],
					page: {
						pageNo: 1
				...
				this.unitList = this.deepClone(unitList).map(item => {
					item.label = item.unit_weight_g && item.unit === 'g' ? item.unit_weight_g + item.unit :
						item.unit_amount && item.unit === 'g' ? item.unit_amount + item.unit : item.unit;
			...
			changeCookMethod(isChange, method) {
				...
				if (this.dietInfo.cook_method_default) {
					let index = this.defaultCookTypes.findIndex(
						item => item.value !== this.cook_method && !typeArr.includes(item.value) && this.dietInfo
						.cook_method_default.indexOf(item.value) === -1
					);
					if (index !== -1 && index !== undefined) {
						this.defaultCookTypes.splice(index, 1);
					}
				} else {
					let index = this.defaultCookTypes.findIndex(item => item.value !== this.cook_method && !typeArr
						.includes(item.value));
					if (index !== -1 && index !== undefined) {
						this.defaultCookTypes.splice(index, 1);
					}
