<template>
	<view class="current-diet-detail" v-if="!authBoxDisplay">
		<view class="title-bar">
			<view class="date" v-if="dietInfo.hdate && dietInfo.htime">
				{{ dietInfo.hdate + ' ' + dietInfo.htime.slice(0, 5) }}
			</view>
		...
		<element-detail ref="elementDetail" :dietInfo="dietInfo" :dietList="dietRecordList" :userInfo="userInfo">
		</element-detail>
		<handler-bar :dietInfo="dietInfo" :cook-method="cook_method" @checkUnit="checkUnit"
			@changeCookMethod="chooseCookType" @changeDiningType="changeDiningType" @changeAmount="changeAmount">
		</handler-bar>
	...
	<bx-auth v-else-if="authBoxDisplay"></bx-auth>
</template>
	...
	export default {
		...
		methods: {
							...
							if (this.dietInfo.dish_type && this.dietInfo.dish_type.indexOf('自选') > -1) {
								if (this.minceListChoose && this.minceListChoose.length > 0) {
									this.minceListChoose.forEach(m => {
										obj.energy = (Number(obj.energy) + m.unit_energy * item.amount)
											.toFixed(1);
										obj.protein = (Number(obj.protein) + Number(m.protein) * item
											.amount).toFixed(1);
										obj.axunge = (Number(obj.axunge) + Number(m.axunge) * item
											.amount).toFixed(1);
										obj.carbohydrate = (Number(obj.carbohydrate) + Number(m
											.carbohydrate) * item.amount).toFixed(1);
										obj.cholesterol = (Number(obj.cholesterol) + Number(m
											.cholesterol) * item.amount).toFixed(1);
										obj.vitamin_a = (Number(obj.vitamin_a) + Number(m.vitamin_a) *
											item.amount).toFixed(1);
										obj.vitamin_d = (Number(obj.vitamin_d) + Number(m.vitamin_d) *
											item.amount).toFixed(1);
										obj.vitamin_e = (Number(obj.vitamin_e) + Number(m.vitamin_e) *
											item.amount).toFixed(1);
										obj.vitamin_k = (Number(obj.vitamin_k) + Number(m.vitamin_k) *
											item.amount).toFixed(1);
										obj.vitamin_b1 = (Number(obj.vitamin_b1) + Number(m
											.vitamin_b1) * item.amount).toFixed(1);
										obj.vitamin_b2 = (Number(obj.vitamin_b2) + Number(m
											.vitamin_b2) * item.amount).toFixed(1);
										obj.vitamin_b3 = (Number(obj.vitamin_b3) + Number(m
											.vitamin_b3) * item.amount).toFixed(1);
										obj.vitamin_b6 = (Number(obj.vitamin_b6) + Number(m
											.vitamin_b6) * item.amount).toFixed(1);
										obj.vitamin_b12 = (Number(obj.vitamin_b12) + Number(m
											.vitamin_b12) * item.amount).toFixed(1);
										obj.folic_acid = (Number(obj.folic_acid) + Number(m
											.folic_acid) * item.amount).toFixed(1);
										obj.vitamin_c = (Number(obj.vitamin_c) + Number(m.vitamin_c) *
											item.amount).toFixed(1);
										obj.element_ca = (Number(obj.element_ca) + Number(m
											.element_ca) * item.amount).toFixed(1);
										obj.element_p = (Number(obj.element_p) + Number(m.element_p) *
											item.amount).toFixed(1);
										obj.element_mg = (Number(obj.element_mg) + Number(m
											.element_mg) * item.amount).toFixed(1);
										obj.element_k = (Number(obj.element_k) + Number(m.element_k) *
											item.amount).toFixed(1);
										obj.element_na = (Number(obj.element_na) + Number(m
											.element_na) * item.amount).toFixed(1);
										obj.element_fe = (Number(obj.element_fe) + Number(m
											.element_fe) * item.amount).toFixed(1);
										obj.element_zn = (Number(obj.element_zn) + Number(m
											.element_zn) * item.amount).toFixed(1);
										obj.element_se = (Number(obj.element_se) + Number(m
											.element_se) * item.amount).toFixed(1);
										obj.element_cu = (Number(obj.element_cu) + Number(m
											.element_cu) * item.amount).toFixed(1);
										obj.element_mn = (Number(obj.element_mn) + Number(m
											.element_mn) * item.amount).toFixed(1);
									});
								}
								if (this.chooseBurdening && this.chooseBurdening.length > 0) {
									this.chooseBurdening.forEach(n => {
										obj.energy = (Number(obj.energy) + n.unit_energy * item.amount)
											.toFixed(1);
										obj.protein = (Number(obj.protein) + Number(n.protein) * item
											.amount).toFixed(1);
										obj.axunge = (Number(obj.axunge) + Number(n.axunge) * item
											.amount).toFixed(1);
										obj.carbohydrate = (Number(obj.carbohydrate) + Number(n
											.carbohydrate) * item.amount).toFixed(1);
										obj.cholesterol = (Number(obj.cholesterol) + Number(n
											.cholesterol) * item.amount).toFixed(1);
										obj.vitamin_a = (Number(obj.vitamin_a) + Number(n.vitamin_a) *
											item.amount).toFixed(1);
										obj.vitamin_d = (Number(obj.vitamin_d) + Number(n.vitamin_d) *
											item.amount).toFixed(1);
										obj.vitamin_e = (Number(obj.vitamin_e) + Number(n.vitamin_e) *
											item.amount).toFixed(1);
										obj.vitamin_k = (Number(obj.vitamin_k) + Number(n.vitamin_k) *
											item.amount).toFixed(1);
										obj.vitamin_b1 = (Number(obj.vitamin_b1) + Number(n
											.vitamin_b1) * item.amount).toFixed(1);
										obj.vitamin_b2 = (Number(obj.vitamin_b2) + Number(n
											.vitamin_b2) * item.amount).toFixed(1);
										obj.vitamin_b3 = (Number(obj.vitamin_b3) + Number(n
											.vitamin_b3) * item.amount).toFixed(1);
										obj.vitamin_b6 = (Number(obj.vitamin_b6) + Number(n
											.vitamin_b6) * item.amount).toFixed(1);
										obj.vitamin_b12 = (Number(obj.vitamin_b12) + Number(n
											.vitamin_b12) * item.amount).toFixed(1);
										obj.folic_acid = (Number(obj.folic_acid) + Number(n
											.folic_acid) * item.amount).toFixed(1);
										obj.vitamin_c = (Number(obj.vitamin_c) + Number(n.vitamin_c) *
											item.amount).toFixed(1);
										obj.element_ca = (Number(obj.element_ca) + Number(n
											.element_ca) * item.amount).toFixed(1);
										obj.element_p = (Number(obj.element_p) + Number(n.element_p) *
											item.amount).toFixed(1);
										obj.element_mg = (Number(obj.element_mg) + Number(n
											.element_mg) * item.amount).toFixed(1);
										obj.element_k = (Number(obj.element_k) + Number(n.element_k) *
											item.amount).toFixed(1);
										obj.element_na = (Number(obj.element_na) + Number(n
											.element_na) * item.amount).toFixed(1);
										obj.element_fe = (Number(obj.element_fe) + Number(n
											.element_fe) * item.amount).toFixed(1);
										obj.element_zn = (Number(obj.element_zn) + Number(n
											.element_zn) * item.amount).toFixed(1);
										obj.element_se = (Number(obj.element_se) + Number(n
											.element_se) * item.amount).toFixed(1);
										obj.element_cu = (Number(obj.element_cu) + Number(n
											.element_cu) * item.amount).toFixed(1);
										obj.element_mn = (Number(obj.element_mn) + Number(n
											.element_mn) * item.amount).toFixed(1);
									});
								}
							} else {
			...
			checkUnit(currentUnit) {
				...
				if (currentUnit.unit === 'g') {
					...
					this.dietInfo.unit_energy = (currentUnit.amount * this.dietInfo.unit_energy) / (this.dietInfo
						.unit_weight_g >= 100 ? this.dietInfo.unit_weight_g : 100);
				}
		...
		onShareAppMessage(res) {
			...
			if (this.userInfo && this.userInfo.no) {
				let no = this.dietInfo.food_no ? this.dietInfo.food_no : this.dietInfo.diet_record_no ? this.dietInfo
					.diet_record_no : this.dietInfo.diet_contents_no;
				if (no) {
					path =
						`/archivesPages/DietDetail/DietDetail?chooseDate=${this.formateDate()}&no=${no}&from=share&invite_user_no=${this.userInfo.userno}`;
				} else {
					return;
				}
