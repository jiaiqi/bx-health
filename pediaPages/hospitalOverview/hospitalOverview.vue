	...
	<view>
		<view class="header-wrap" v-if="storeInfo && storeInfo.id" :style="{ 'background-image': 'url(' + getImagePath(storeInfo.image) + ')' }">
			<view class="hospital-top">
				...
				<view class="right" v-if="storeInfo.telephone"><text class="cuIcon-phone" @click="makePhoneCall"></text></view>
			</view>
			<view class="introduction">
				<view class="title">医院简介</view>
				<view class="content">
					<view class="rich-text">{{ introduction }}</view>
			...
			<view class="introduction">
				...
				<view class="content">
					...
					<view class="depart-box">
						<text class="depart-item" v-for="item in deptList" :key="item.no">{{ item.dept_name }}</text>
					</view>
					<!-- </u-read-more> -->
				</view>
			</view>
			<view class="introduction">
				...
				<view class="content">
					<view class="professor-box">
						<view class="professor-item" v-for="item in staffList" :key="item.docCode" @click="toDocotrDetail(item)">
							...
							<view class="doc-info">
								<view class="top">
									<text class="doc-name">{{ item.person_name }}</text>
									<view class="titleDn">{{ item.titleDn }}</view>
								</view>
								<view class="center">
									<view class="depart-name">科室：{{ item.deptName || '-' }}</view>
								</view>
								<view class="bottom">{{ item.staff_introduction }}</view>
							</view>
						</view>
					</view>
				</view>
...
export default {
	...
	computed: {
		introduction() {
			if (this.hospitalData.introduction.length > 50) {
				return this.hospitalData.introduction.slice(0, 50) + '...';
			}
		},
		departmentList() {
	...
	methods: {
		...
		toPages(type) {},
		selectDepartList() {
			let url = this.getServiceUrl('health', 'srvhealth_store_dept_select', 'select');
			let req = {
