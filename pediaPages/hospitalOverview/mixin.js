			...
			this.$fetch('multi', 'select', req, 'health').then(res => {
				if (res.success && res.data.length === 3) {
					...
					if (data[1].state === 'SUCCESS' && Array.isArray(data[1].data) && data[1].data.length >
						0) {
						let storeUserInfo = data[1].data[0]
						this.storeNum = storeUserInfo.kefu_kefu_unread_msg || 0
					}

					if (data[2].state === 'SUCCESS' && Array.isArray(data[2].data) && data[2].data.length >
						0) {
						// kefuNum
						let sessionInfo = data[2].data[0]
						this.kefuNum = sessionInfo.kefu_kefu_unread_msg || ''
					}
				}
			})
		}
