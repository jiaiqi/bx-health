	...
	<view class="pregnant_wrap">
		<view class="pregnant-top">
			...
			<view class="pregnant-time">
				...
				<view @click="openCale" class="pregnant-time-right"><u-icon size="50" name="calendar"></u-icon></view>
			</view>
		</view>
				...
				<view class="pregnant-main-top-item">
					<view class="pregnant-main-top-item-w">
						<scroll-view :scroll-with-animation="true" style="height: 60upx;" scroll-x :scroll-left="scrollMenuLeft">
							<text
								@click="chooseClasify(item, index)"
								:class="currClasifyIndex === index ? 'active-clasify' : ''"
				...
				<view class="cu-timeline">
					...
					<view v-for="(item, index) in fetusTimeLienData" :key="index" :class="item.isCurrent ? 'text-green' : ''" class="cu-item cur">
						...
						<view v-show="item.isOpen" class="fetus-top">
							<view class="fetus-top-tit"><text>胎儿发育</text></view>
							<view class="fetus-top-main">
								<view class="fetus-top-main-left"><image src="/pregnancy/static/img/1.png" mode=""></image></view>
							...
							<view class="fetus-top-bot">
								<view class="fetus-top-bot-t"><text>概览</text></view>
								...
								<view class="diet-main-top">
									...
									<view class="diet-main-item-wrap">
										<view @click="toFoodsDetail('recommend')" v-for="(rec, i) in item.recommend" :key="i" class="diet-main-item">
											...
											<view class="diet-main-item-b">{{ rec.name }}</view>
										</view>
									</view>
								</view>
								<view class="diet-main-top">
									...
									<view class="diet-main-item-wrap">
										<view @click="toFoodsDetail('taboo')" v-for="(dietab, o) in item.taboo" :key="o" class="diet-main-item taboo-item">
											...
											<view class="diet-main-item-b">{{ dietab.name }}</view>
										</view>
									</view>
								</view>
							</view>
					...
					<view v-for="(item, index) in inspectTimeLienData" :key="index" class="cu-item cur" :class="item.isCurrent ? 'text-green' : ''">
						...
						<view v-show="item.isOpen">
							<view class="pre-chunk-top pre-chunk-bot matter-wrap-main">
								<view class="pre-chunk-top-notice"><text>待办事项</text></view>
								<view class="pre-chunk-top-backlog-cont">
									<view v-for="(sing, i) in item.backlog" :key="index" @click="toPreDetail('no')" class="pre-chunk-top-backlog-cont-item">
										...
										<view class="pre-chunk-top-backlog-cont-item-r"><text>查看详情</text></view>
									</view>
								</view>
							</view>
							<view class="pre-chunk-top pre-chunk-bot matter-wrap-main">
								<view class="pre-chunk-top-notice"><text>已办事项</text></view>
								<view class="pre-chunk-top-backlog-cont">
				...
				<view class="cu-timeline">
					...
					<view v-for="(item, index) in symptomTimeLienData" :key="index" class="cu-item cur">
						...
						<view v-show="item.isOpen" class="symptom_main">
							<view class="symptom_main_top"><text>可能出现得症状</text></view>
							<view class="symptom_main_bot">
								<view @click="toSymDetail(sym)" v-for="(sym, n) in item.symptom" class="symptom_main_item">
									...
									<view class="symptom_main_item_bot"><text>查看详情</text></view>
								</view>
							</view>
						</view>
					</view>
						...
						<view class="content shadow-blur" :class="item.isCurrent ? 'inspect-bg' : 'bg-yellow'">
							<view class="inspect_wrap_top_left">
								...
								<view class="inspect_wrap_top_left_c">
									<view class="inspect_wrap_top_left_c_l"><text>产检重点：</text></view>
							...
							<view class="inspect_wrap_top_rig"><u-icon name="arrow-right"></u-icon></view>
						</view>
					</view>
				</view>
			</view>
						...
						<view v-for="(week, i) in item.child" class="cu-item " :class="week.isCurrent ? 'text-green' : ''">
							<view class="content " :class="week.isCurrent ? 'bg-green shadow-blur' : 'bg-cyan'">
								...
								<view class="content-item-cen">
									<view @click="toDetail(week)" class="content-item-cen-l">详情</view>
								...
								<view class="pre-chunk-top">
									<view v-for="(con, k) in week.content" :key="k" class="pre-chunk-top-item">
										...
										<view class="pre-chunk-item-wrap">
											...
											<view
												@click="toFoodsDetail(pre.type)"
												v-for="(pre, p) in con.child"
												:class="pre.type === 'recom' ? 'pass-pre-chunk' : ''"
												class="pre-chunk"
											>
												<view class="pre-chunk-top">
												...
												<view class="pre-chunk-item-bot">{{ pre.tit }}</view>
											</view>
										</view>
									</view>
								</view>
									...
									<view class="pre-chunk-top-backlog-cont">
										<view @click="toPreDetail('no')" class="pre-chunk-top-backlog-cont-item">
											<view class="pre-chunk-top-backlog-cont-item-l">
												<text>1.建档</text>
												<text>
													建立母子健康档案意味着各次产检都会在 这家医院进行，宝宝也会在这里出生。一般来说 建档主要为了确定孕周、推算预产期、评估妊娠
													风险等。
												</text>
											</view>
											<view class="pre-chunk-top-backlog-cont-item-r"><text>查看详情</text></view>
		...
		<view v-else-if="current_tit === 'prepare'" class="pregnant-main-pregnancy"><text>备孕</text></view>
		<view v-else-if="current_tit === 'postpartum'" class="pregnant-main-delivery"><text>产后</text></view>
		<view class="cu-modal" :class="{ show: show }">
			<view class="cu-dialog">
				<view class="pre-model-wrap">
					<view class="pre-top"><text>确认下列注意事项是否进行?</text></view>
					<view class="pre-content">
						<view v-for="(curr, i) in current_pre.child" class="pre-content-item">
							<view class="pre-content-top">
								<view class="pre-left"><image :src="curr.image" mode=""></image></view>
					...
					<view class="pre-bot">
						<view @click="botBtn('true')" class="pre-bot-left">是</view>
						<view @click="botBtn('false')" class="pre-bot-left">否</view>
					</view>
				</view>
			</view>
		</view>
		<view class="cu-modal bottom-modal" :class="{ show: menuIsShow }">
			<view class="cu-dialog">
				<view class="pregnant-main-top-item-poup">
					<text @click="chooseClasify(item, index)" :class="currClasifyIndex === index ? 'active-clasify' : ''" v-for="(item, index) in classifyData" :key="index">
						{{ item.name }}
					</text>
				</view>
			</view>
		</view>
		<view class="cu-modal" :class="{ show: showTimeSignPicker }">
			<view class="cu-dialog"><bx-date-stamp v-show="showTimeSignPicker" ref="ren" :headerBar="true" :isMarkDays="false" @onDayClick="onDayClick"></bx-date-stamp></view>
		</view>
	</view>
</template>

<script>
import bxDateStamp from '@/pediaPages/components/bx-date-stamp/bx-date-stamp.vue';
export default {
	name: 'pregnant',
							...
							content: [
								{
									...
									child: [
										...
										},
										{
											...
											type: 'taboo',
											content:
												'准妈妈不能喝白酒，白酒中含有一定量的酒精，酒精会通过血液循环到达胎盘，会影响薄薄的智力发育，甚至会造成宝宝畸形。准妈妈摄入酒精过多会刺激子宫收缩，造成流产、早产等后果',
											isOpen: false,
		...
		calcTimeQuantum(time) {
			...
			matterTimeLineData.forEach((matter, t) => {
				this.$set(
					matter,
					'start_time',
					this.formateDate(startTime * (matter.num - 1) * 7 * 60 * 60 * 1000 == 0 ? startTime : startTime + (matter.num - 1) * 7 * 24 * 60 * 60 * 1000)
				);
