<template>
	<view class="health-archive-wrap"
		...
		<view class="top text-bold">
			<view class="left">
				<view class="avatar" @click="showUserListPopup = true">
					<image class="avatar" :src="getUserImage(vuex_userInfo)"></image>
				</view>
			...
			<view class="content">
				<view class="score-item" @click="toPages('score-compose')">
					...
					<text class="value text-blue" v-if="healthTotalScore || healthTotalScore === 0">
						...
						<text class="float"
							v-if="totalScore && totalScore.digit">.{{ totalScore.digit | fixed1 }}</text>
					</text>
				...
				<view class="score-item" @click="showAdvice = !showAdvice">
					...
					<text class="value  text-cyan">
						...
						<text class="float"
							v-if="todayScore && todayScore.digit">.{{ todayScore.digit | fixed1 }}</text>
		...
		<view class="health-archive-item health-todos">
			...
			<view class="empty-data" v-if="!isLoadPlan && planList.length === 0">
				<view class="cu-load" :class="'loading'"></view>
			</view>
			<view class="content todo-list" v-if="planList.length > 0">
				<view class="todo-item" v-for="(item, index) in planList" :key="index"
		...
		<view class="health-archive-item ">
			...
			<view class="content inspection-report">
				<view class="report-item" v-for="item in inspectReportRecord" :key="item.id" @click="toRecord(item)">
					<view class="images">
						<image class="image" src="../../static/xuehzi.jpg" mode="aspectFill"></image>
					</view>
		...
		<view class="cu-modal" :class="{ show: showUserListPopup }" @tap="showUserListPopup = false">
			<view class="cu-dialog" @tap.stop="">
				<view class="user-list">
					<view class="user-item" @click="switchUser(item)" v-for="item in vuex_userList" :key="item.id"
						:class="{ 'text-blue': item.name === vuex_userInfo.name }">
						...
						<button class="cu-btn bg-blue settings" @click.stop="editUserInfo(userInfo)"
							v-if="item.name === vuex_userInfo.name">
							<view class="cuIcon-settings"></view>
						</button>
					</view>
				</view>
				<view class="button-box"><button class="cu-btn bg-white text-blue" @click="toAddPages">添加新用户</button>
				</view>
			</view>
		</view>
		<view class="cu-modal bottom-modal" :class="{ show: showUserHealtManagePopup }"
			@tap="showUserHealtManagePopup = false">
			<view class="cu-dialog health-item" @tap.stop="">
				<bx-checkbox-group max="5" checkboxMode="button" v-model="checkedList">
					<bx-checkbox v-model="item.checked" v-for="item in checkboxList" :key="item.value"
						:name="item.label">{{ item.label }}</bx-checkbox>
				</bx-checkbox-group>
	...
	export default {
		data() {
			return {
				...
				configCols: {
					...
					display: true,
					option_list_v2: {
						refed_col: 'daq_survey_activity_no',
						srv_app: 'health',
		...
		computed: {
			...
			bmi() {
				if (this.vuex_userInfo.weight && this.vuex_userInfo.height) {
					return Number((Number(this.vuex_userInfo.weight) / Math.pow(Number(this.vuex_userInfo.height) / 100,
						2)).toFixed(1));
				}
			},
			age() {
		...
		methods: {
			...
			async getplanList() {
				...
				if (Array.isArray(res.data.data)) {
					this.planList = res.data.data
						...
						.filter(item => dayjs(item.start_date).add(item.take_days, 'day') >= dayjs().subtract(1,
							'day'));
				}
			},
			showAddRecordLayout() {
			...
			confirmTree(e) {
				if (e.item && e.item.jy_no) {
					...
					if (e.item.daq_survey_activity_no) {
						...
						fieldsCond[fieldsCond.findIndex(item => item.column === 'report_daq_survey_activity_no')].value = e
							.item.daq_survey_activity_no;
						url =
			...
			toRecord(item) {
				...
				let url =
					`/archivesPages/report/report?serviceName=srvhealth_examination_report_select&type=detail&fieldsCond=${encodeURIComponent(JSON.stringify(fieldsCond))}`;
				uni.navigateTo({
					url: url
				});
				// }
			...
			async selectInspectionReport() {
				...
				let req = {
					...
					condition: [{
							colName: 'examinate_account',
							ruleType: 'eq',
							value: this.vuex_userInfo.userno
						},
						{
							colName: 'examinate_person_no',
							ruleType: 'eq',
							value: this.vuex_userInfo.no
						}
					]
				...
				if (Array.isArray(energyInfo) && energyInfo.length > 0) {
					energyInfo.forEach(ele => {
						if (ele.key === 'protein') {
							...
							if (ele.EAR < ele.value && ele.value < ele.EAR * 2) {} else if (ele.EAR > ele
								.value) {
				...
				energyListWrap = this.energyListWrap.map(item => {
					item.matterList = item.matterList.map(ele => {
						energyListWrap.forEach(ene => {
							if (ene.nutrient === ele.name || ene.nutrient.indexOf(ele.name) !==
								-1) {
								...
								if (ele.name === '蛋白') {
									ele.EAR = ene.val_rni ? ene.val_rni * self.vuex_userInfo
										.weight : ene.val_ear ? ene.val_ear * self
										.vuex_userInfo.weight : ele.EAR * self.vuex_userInfo
										.weight;
									ele.UL = 0;
								}
							} else {
								if (ele.name === '脂肪') {
									ele.EAR = Number((self.vuex_userInfo.weight * 50 * 0.2) /
										9).toFixed(2);
									ele.UL = 0;
								}
								if (ele.name === '碳水') {
				...
				eleArr = eleArr.map(ele => {
					dietRecord.forEach(diet => {
						foodType.forEach(food => {
							if ((food.food_no && food.food_no === diet.diet_contents_no) || (
									food.meal_no && food.meal_no === diet.mixed_food_no)) {
								let ratio = 1;
								if (food['unit'] === 'g') {
									ratio = (diet.unit_weight_g * diet.amount) / food
										.unit_amount;
								}
						...
						self.energyListWrap.forEach(energy => {
							energy.matterList.forEach(mat => {
								if (item.nutrient === mat.name || item.nutrient.indexOf(mat
									...
									if (mat.name === '蛋白') {
										mat.EAR = item.val_rni ? item.val_rni * self
											.vuex_userInfo.weight : item.val_ear ? item
											.val_ear * self.vuex_userInfo.weight : mat.EAR *
											self.vuex_userInfo.weight;
										mat.UL = 0;
									}
								} else {
									if (mat.name === '脂肪') {
										mat.EAR = Number((self.vuex_userInfo.weight * 50 *
											0.2) / 9).toFixed(2);
										mat.UL = 0;
									}
									if (mat.name === '碳水') {
			...
			async getCheckboxList() {
				...
				let req = {
					...
					condition: [{
						...
						value: 'srvhealth_person_info_select',
						ruleType: 'eq'
					}, {
						colName: 'use_type',
						value: 'list',
							...
							srv_cols.forEach(item => {
								if (item.columns === 'requirement') {
									this.checkboxList = item.option_list_v2.map(item => {
										item['checked'] = false;
										if (Array.isArray(requirement) && requirement.includes(item
												.value) && !this.selectedTags.find(s => s.value ===
												item.value)) {
			...
			updateUserInfo() {
				...
				uni.getUserInfo({
					...
					success: function(user) {
						...
						if (self.vuex_userInfo && self.vuex_userInfo.no && rawData.headimgurl !== self
							.vuex_userInfo.profile_url) {
							self.updateUserProfile(rawData.headimgurl, self.vuex_userInfo.no, user.userInfo
								.nickName).then(_ => {
