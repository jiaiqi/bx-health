	...
	<view class="page-wrap">
		...
		<view class="vaccine-list">
			<view class="vaccine-item animation-slide-top " v-for="(item,index) in list" :key="item.id">
				<view class="badge-ribbon" v-if="current===0">
				...
				<view class="badge-ribbon notice" v-if="current===1">
				</view>
				<view class="content">
					<view class="name">{{item.appoint_name}}</view>
					<view class="desc " v-if="current===0">
...
<script>
	...
	export default {
		...
		methods: {
			changeTab(index) {
				this.current = index;
				this.getVaccineRecord('refresh', index)
			},
			DateChange(e) {
			...
			cancel(e) {
				// 取消预约
				let req = [{
					"serviceName": "srvhealth_store_vaccination_appoint_record_delete",
					"condition": [{
						"colName": "id",
						"ruleType": "eq",
						"value": e.id
				...
				uni.showModal({
					...
					success(res) {
						if (res.confirm) {
							self.$fetch('operate', 'srvhealth_store_vaccination_appoint_record_delete', req,
								'health').then(result => {
								if (result.success) {
									uni.showToast({
										title: '已成功取消预约'
