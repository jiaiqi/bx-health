  ...
  <view class="balanced-diet">
      ...
      <view class="header-wrap">
        ...
        <view class="user-info" @click.stop="showUserList = !showUserList">
          ...
          <view class="user-name" data-event="showUserList">
            <text class="text" data-event="showUserList">{{
              userInfo.name
            }}</text>
            <u-icon
              name="arrow-down-fill"
              size="28"
              :class="{ active: showUserList }"
            ></u-icon>
          </view>
          <view class="user-list" :class="{ active: showUserList }">
            ...
            <view class="menu-item" @click.stop="clickUserMenu('regulate')"
              >人员管理</view
            >
          </view>
        </view>
    ...
    <view class="diet-wrap">
      ...
      <view class="main-box">
        <view class="main-content main-content-t">
          ...
          <view class="energy-item">
            ...
            <view class="number">{{
              basicOut ? parseFloat(basicOut).toFixed(1) : "0"
            }}</view>
          </view>
          <view class="operate">=</view>
          <view class="energy-item">
            ...
            <view
              class="number text-red"
              style="display: flex; width: 90px; justify-content: space-between"
            >
              <text style="flex: 1">{{
                parseFloat(energyChange) > 0
                  ? `+${parseFloat(energyChange).toFixed(1)}`
                  : parseFloat(energyChange).toFixed(1)
              }}</text>
      ...
      <view class="main-box main-box-plus">
        ...
        <view v-for="(item, index) in energyList" :key="index" class="main-box">
          ...
          <view class="main-content ele-content" v-if="energyList.length > 0">
            <view
              v-for="(alone, i) in item.matterList"
              :key="i"
              class="ele-item"
              @click="toDetail(alone, item)"
            >
              <view
                class="ele-item-wrap"
                v-if="
                  subIndex === 1
                    ? alone.value < alone.EAR || alone.value > alone.UL
                    : true
                "
              >
                ...
                <view class="bg-white probar">
                  <view class="cu-progress progress-bar radius pro-rad">
                        ...
                        {{
                          alone.value === 0
                            ? alone.shortName === "E"
                              ...
                              : "0"
                            : alone.shortName === "E"
                            ? alone.value.toFixed(2) + "mg/d"
                            : alone.value.toFixed(2)
                        }}
                      </view>
                    </view>
                    <view
                      class="bg-grey EAR"
                      :style="{
                        width: alone.left_width
                          ? alone.left_width + 'px'
                          : '33%',
                      }"
                    >
                      <view class="after">{{
                        alone.left_width ? alone.EAR : ""
                      }}</view>
                    </view>
                    <view
                      class="bg-olive regular"
                      :style="{
                        width: alone.center_width
                          ? alone.center_width + 'px'
                          : '33%',
                    ...
                    <view
                      ...
                      :style="{
                        width:
                          alone.UL && alone.right_width
                            ? alone.right_width + 'px'
                            : !alone.UL
                            ? 0
                            : '33%',
                      }"
                    >
                      <view
                        ...
                        :class="{
                          ...
                          normal:
                            alone.value >= alone.EAR &&
                            (alone.value <= alone.UL || !alone.UL),
                      ...
                      >
                        {{
                          alone.value < alone.EAR
                            ? "不足"
                            : alone.UL && alone.value > alone.UL
                            ? "过多"
                            : "正常"
                        }}
                      </view>
                      <view class="after">{{
                        alone.right_width && alone.UL ? alone.UL : ""
                      }}</view>
                    </view>
                  </view>
                </view>
      ...
      <view class="main-box symptom">
        ...
        <view
          class="record-box"
          @click.self="clickDietBox"
          :class="{ showall: foodListDisplay, readmore: dietRecord.length > 3 }"
        >
          <view class="table" v-if="dietRecord">
            <view
              class="no-data"
              v-if="!dietRecord || dietRecord.length === 0"
              @click="toPages('food')"
            >
              <view
                class="cuIcon-add text-black"
              ...
              <view class="readonly" @click="clickDietRecordItem(item)">
                <view class="img"
                  ><u-image
                    width="100%"
                    height="100%"
                    :src="getDownloadPath(item)"
                  ></u-image
                ></view>
                <view class="column center">
                  <view class="name">{{ item.name }}</view>
                  <view class="number">{{
                    item.amount * item.unit_weight_g + item.unit
                  }}</view>
                </view>
                <view class="column time">{{
                  item.htime ? item.htime.slice(0, 5) : ""
                }}</view>
                <view class="column">
                  <text class="heat">
      ...
      <view class="main-box symptom">
        ...
        >
          <view class="table">
            <view
              class="no-data"
              v-if="!sportsRecord || sportsRecord.length === 0"
              @click="toPages('sport')"
            >
              <view
                class="cuIcon-add text-black"
            ...
            >
              <view
                ...
                @click="
                  (showEditModal = true),
                    (currentRecord = deepClone(item)),
                    (currentRecordType = 'sport')
                "
              >
                <view class="img"
                  ><u-image
      ...
      <view class="main-box symptom">
        ...
        <view class="symptom-box">
          <view
            ...
            :key="index"
            v-show="item.disease_name"
            >{{ item.disease_name }}</view
          >
    ...
    <view
      ...
      @click.self="(showEditModal = false), (currentRecord = null)"
    >
      <view class="cu-dialog" v-if="currentRecord">
        <view class="title-bar">
          <view
            class="btn"
            @click="(showEditModal = false), (currentRecord = null)"
            >取消</view
          >
          <view class="date">{{
        ...
        <view class="diet-info">
          ...
          <view class="info">
            ...
            <view class="weight">
              ...
              /{{
                currentRecord.unit_weight_g
                  ? currentRecord.amount * currentRecord.unit_weight_g
    ...
    <mx-date-picker
      ...
      @cancel="onSelected"
    />
    <view class="add-button" @click="clickAddButton"
    ...
    <u-popup v-model="showPopup" mode="bottom" border-radius="50">
      <view class="popup-box">
        <view class="icon-item" @click="toPages('food')">
          <image
            src="@/otherPages/static/icon/yinshi.png"
            mode=""
            class="icon"
          ></image>
        ...
        <view class="icon-item" @click="toPages('sport')">
          <image
            src="@/otherPages/static/icon/yundong.png"
            mode=""
            class="icon"
          ></image>
        ...
        <view class="icon-item" @click="toPages('weight')">
          <image
            src="@/otherPages/static/icon/tizhong.png"
            mode=""
            class="icon"
          ></image>
        ...
        <view class="icon-item" @click="toPages('sleep')">
          <image
            src="@/otherPages/static/icon/sleep.png"
            mode=""
            class="icon"
          ></image>
        ...
        <view class="icon-item" @click="toPages('heartRate')">
          <image
            src="@/otherPages/static/icon/xinlv.png"
            mode=""
            class="icon"
          ></image>
        ...
        <view class="icon-item" @click="toPages('pressure')">
          <image
            src="@/otherPages/static/icon/xueya.png"
            mode=""
            class="icon"
          ></image>
        ...
        <view class="icon-item" @click="toPages('oxygen')">
          <image
            src="@/otherPages/static/icon/xueyang.png"
            mode=""
            class="icon"
          ></image>
        ...
        <view class="icon-item" @click="toPages('glucose')">
          <image
            src="@/otherPages/static/icon/xuetang.png"
            mode=""
            class="icon"
          ></image>
      ...
      <view class="close-icon"
        ><text @click="showPopup = false" class="cuIcon-close"></text
      ></view>
    </u-popup>
    <view
      @click.self="closeDay"
...
export default {
  ...
  computed: {
    ...
    energyChange() {
      return (
        Number(this.dietIn) - Number(this.sportOut) - Number(this.basicOut)
      );
    },
    age() {
  ...
  methods: {
    ...
    toAddDiet() {
      ...
      const url =
        "/otherPages/dietSelect/dietSelect?condType=" +
        encodeURIComponent(JSON.stringify(condType));
    ...
    async getFoodList(diet) {
      ...
      let req = {
        ...
        colNames: ["*"],
        condition: [
          { colName: "name", ruleType: "in", value: foodNameList.toString() },
        ],
      };
    ...
    async buildChartData(e) {
      ...
      energy.forEach((item) => {
        ...
        resultData.chartOption.legend.data = diet
          .map((item) => item.name)
          .concat(lineList);
        resultData.chartOption.xAxis.data = item.matterList.map(
          (item) => item.key
        );
    ...
    toDetail(e, item) {
      ...
      uni.navigateTo({
        url:
          "/otherPages/elementDetail/elementDetail?data=" +
          JSON.stringify(e) +
          "&user_no=" +
          this.loginUserInfo.user_no +
          "&date=" +
          this.selectDate,
      });
    },
      ...
      if (Array.isArray(symptomList) && symptomList.length > 0) {
        ...
        if (symptomNoStr) {
          const url = this.getServiceUrl(
            "health",
            "srvhealth_symptoms_disease_select",
            "select"
          );
          const req = {
      ...
      switch (this.subIndex) {
        ...
        case 2:
          ...
          this.energyList = energyListWrap.filter((item) => {
            item.matterList = item.matterList.filter((ele) => {
              return ele.UL
                ? ele.value >= ele.EAR && ele.value <= ele.UL
                : ele.value >= ele.EAR;
            });
            return item.matterList.length > 0;
          });
        ...
        success(res) {
          if (res.confirm) {
            ...
            if (type === "food") {
              ...
              url = self.getServiceUrl(
                "health",
                "srvhealth_body_activity_record_delete",
                "operate"
              );
            }
            let req = [
    ...
    updateData(data, type) {
      ...
      if (type === "food") {
        ...
        url = this.getServiceUrl(
          "health",
          "srvhealth_body_activity_record_update",
          "operate"
        );
        req = [
          {
    ...
    async getNutrientRecommended() {
      ...
      if (Array.isArray(res.data.data) && res.data.data.length > 0) {
        let result = res.data.data.filter((item) => {
          if (
            (item.sex && item.sex.indexOf(self.userInfo.sex) !== -1) ||
            !item.sex
          ) {
    ...
    async getDietRecord(chooseDate = null) {
      ...
      energyList.forEach((item) => {
        item.matterList.forEach((mat) => {
          ...
          if (
            mat.center_width * 3 < mat.left_width ||
            mat.center_width * 3 < mat.right_width
          ) {
                  ...
                  chooseFood.forEach((fod) => {
                    for (let a in fod) {
                      if (mat.key && mat.key == a) {
                        ...
                        mat.value =
                          Number(mat.value) +
                          Number(fod.amount) * Number(fod[a]);
                        /**
                        ...
                        if (
                          mat.UL &&
                          mat.value &&
                          mat.value > (mat.UL - mat.EAR) * 4
                        ) {
                          mat["value_left"] = (120 * 0.9) / 2;
                          mat["left_width"] =
                            (mat.EAR * 120) / ((mat.value * 2) / 0.9);
                          mat["center_width"] =
                            ((mat.UL - mat.EAR) * 120) /
                            ((mat.value * 2) / 0.9);
                          mat["right_width"] =
                            120 - mat["left_width"] - mat["center_width"];
                        } else {
                          if (!mat.UL) {
                            mat["right_width"] = 0;
                            mat["left_width"] = 50;
                            ...
                            if (mat.value <= mat.EAR) {
                              mat["value_left"] =
                                (mat["left_width"] * mat.value) / mat.EAR;
                            } else {
                              mat["value_left"] =
                                (mat["left_width"] * mat.value) / mat.EAR;
                          ...
                          if (mat.value === 0 || mat.value === "0") {
                            ...
                            mat["left_width"] =
                              (120 * mat.EAR) / (mat.EAR + mat.UL);
                            mat["right_width"] =
                              (120 * mat.EAR) / (mat.EAR + mat.UL) > 30
                                ? (120 * mat.EAR) / (mat.EAR + mat.UL)
                                : 30;
                            ...
                            mat["center_width"] =
                              120 - mat["left_width"] - mat["right_width"];
                            ...
                            if (mat.value >= mat.EAR && mat.value <= mat.UL) {
                              mat["value_left"] =
                                mat["left_width"] +
                                (mat["center_width"] * (mat.value - mat.EAR)) /
                                  (mat.UL - mat.EAR);
                            }
                            if (mat.value > mat.UL) {
                              mat["value_left"] =
                                mat["left_width"] +
                                mat["center_width"] +
                                (mat["right_width"] * (mat.value - mat.UL)) /
                                  mat.EAR;
                            }
    ...
    async getSportType(cond) {
      ...
      let url = this.getServiceUrl(
        "health",
        "srvhealth_body_activity_contents_select",
        "select"
      );
      let req = {
    ...
    toPages(e) {
      ...
      switch (e) {
        case "food":
          ...
          url =
            "/pages/dietSelect/dietSelect?condType=" +
            encodeURIComponent(JSON.stringify(condType));
          url =
            "/otherPages/dietSelect/dietSelect?condType=" +
            encodeURIComponent(JSON.stringify(condType));
    ...
    async getCurrUserInfo() {
      const url = self.getServiceUrl(
        "health",
        "srvhealth_person_info_select",
        "select"
      );
