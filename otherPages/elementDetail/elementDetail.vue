  ...
  <view class="element-wrap">
    <view class="element-box">
      <view class="element-name">
        <text class="text"
          >{{ elementData.label }}({{ elementData.units }})</text
        >
      </view>
        ...
        <view class="probar">
          <view class="cu-progress progress-bar radius">
            <view
              ...
              class="pointer"
              :text="
                elementData.value === 0 ? '0' : elementData.value.toFixed(2)
              "
            ...
            <view
              class="bg-grey EAR"
              :style="{
                width: elementData.left_width
                  ? elementData.left_width * 2 + 'px'
                  : '33%',
              }"
            >
              <view class="after">{{
                elementData.left_width ? elementData.EAR : ""
              }}</view>
            </view>
            <view
              class="bg-olive regular"
              :style="{
                width: elementData.center_width
                  ? elementData.center_width * 2 + 'px'
                  : '33%',
            ...
            <view
              ...
              :style="{
                width:
                  elementData.UL && elementData.right_width
                    ? elementData.right_width * 2 + 'px'
                    : !elementData.UL
                    ? 0
                    : '33%',
              }"
            >
              <view class="after">{{
                elementData.right_width && elementData.UL ? elementData.UL : ""
              }}</view>
              <view
                class="before"
                :class="{
                  ...
                  normal:
                    elementData.value >= elementData.EAR &&
                    (elementData.value <= elementData.UL || !elementData.UL),
              ...
              >
                {{
                  elementData.value < elementData.EAR
                    ? "不足"
                    : elementData.UL && elementData.value > elementData.UL
                    ? "过多"
                    : "正常"
                }}
              </view>
    ...
    <view class="element-box">
      ...
      <view class="cu-list">
        ...
        >
          ...
          <view class="column">{{
            (item.amount * item[elementData.key]).toFixed(1) + elementData.units
          }}</view>
          <view class="column">{{ item.amount * item.unit_energy }}千卡</view>
          <view class="move column"
            ><view class="" @click="deleteItem(item)">删除</view></view
          >
        </view>
    ...
    >
      <view class="cu-dialog" v-if="currentDiet">
        <view class="title-bar">
          ...
          <view class="date">{{
            currentDiet.hdate + " " + currentDiet.htime
          }}</view>
        ...
        <view class="diet-info">
          ...
          <view class="info">
            ...
            <view class="element"
              >{{ elementData.label }}:{{
                (currentDiet.amount * currentDiet[elementData.key]).toFixed(1)
              }}{{ elementData.units }}</view
            >
            <view class="amount">
              数量:
  ...
  methods: {
    buildElemetBarData(e) {
      ...
      if (
        dietInfo.UL &&
        dietInfo.value &&
        dietInfo.value > (dietInfo.UL - dietInfo.EAR) * 4
      ) {
        dietInfo["value_left"] = (120 * 0.9) / 2;
        dietInfo["left_width"] =
          (dietInfo.EAR * 120) / ((dietInfo.value * 2) / 0.9);
        dietInfo["center_width"] =
          ((dietInfo.UL - dietInfo.EAR) * 120) / ((dietInfo.value * 2) / 0.9);
        dietInfo["right_width"] =
          120 - dietInfo["left_width"] - dietInfo["center_width"];
      } else {
        if (!dietInfo.UL) {
          ...
          if (dietInfo.value <= dietInfo.EAR) {
            dietInfo["value_left"] =
              (dietInfo["left_width"] * dietInfo.value) / dietInfo.EAR;
          } else {
            dietInfo["value_left"] =
              (dietInfo["left_width"] * dietInfo.value) / dietInfo.EAR;
          }
        ...
        if (dietInfo.value === 0 || dietInfo.value === "0") {
          ...
          dietInfo["value_left"] =
            (40 * dietInfo.value) / (dietInfo.UL - dietInfo.EAR);
          dietInfo["left_width"] =
            (120 * dietInfo.EAR) / (dietInfo.EAR + dietInfo.UL);
          dietInfo["right_width"] =
            (120 * dietInfo.EAR) / (dietInfo.EAR + dietInfo.UL) > 30
              ? (120 * dietInfo.EAR) / (dietInfo.EAR + dietInfo.UL)
              : 30;
          ...
          dietInfo["center_width"] =
            120 - dietInfo["left_width"] - dietInfo["right_width"];
          if (dietInfo.value < dietInfo.EAR) {
            dietInfo["value_left"] =
              (dietInfo["left_width"] * dietInfo.value) / dietInfo.EAR;
          }
          if (dietInfo.value >= dietInfo.EAR && dietInfo.value <= dietInfo.UL) {
            dietInfo["value_left"] =
              dietInfo["left_width"] +
              (dietInfo["center_width"] * (dietInfo.value - dietInfo.EAR)) /
                (dietInfo.UL - dietInfo.EAR);
          }
          if (dietInfo.value > dietInfo.UL) {
            dietInfo["value_left"] =
              dietInfo["left_width"] +
              dietInfo["center_width"] +
              (dietInfo["right_width"] * (dietInfo.value - dietInfo.UL)) /
                dietInfo.EAR;
          }
        }
        if (dietInfo["value_left"] > 110) {
    ...
    async getDietRecord() {
      ...
      if (res.data.state === "SUCCESS") {
        ...
        this.dietRecord = resultData.sort((a, b) => {
          return (
            b[this.elementData.key] * b.amount -
            a[this.elementData.key] * a.amount
          );
        });
        let dietNameList = [];
        let dietList = [];
    ...
    touchPie(e) {
      canvaPie.showToolTip(e, {
        format: function (item) {
          ...
          return (
            amount * unit_weight_g +
            unit +
            item.name +
            "提供:" +
            Number(amount * item.data).toFixed(1) +
            _self.elementData.units.split("/")[0] +
            " " +
            _self.elementData.label
          );
        },
      });
