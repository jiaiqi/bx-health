<template>
	<view class="contents">
				...
				<view class="box">
					<view class="cu-bar search bg-white">
						<view class="search-form round">
							...
							<input @input="inputChange" :adjust-position="false" v-model="searchValue"
								@keyup.enter="searchStart" type="text" :placeholder="searchInputValue"
								confirm-type="search" />
						</view>
					</view>
				</view>
		...
		<view class="handler-action" v-if="pageType !== 'sport'">
			<text class="margin-right" @click="showModal('sort')"
			...
			<text @click="showModal('filter')"
				:class="{ 'text-cyan text-bold': filterVal && filterVal.indexOf('全部') === -1 }">
				<text>筛选</text>
				<text class="cuIcon-filter"></text>
				<text v-if="filterVal && filterVal.indexOf('全部') === -1"
					style="font-size: 12px;">({{ filterVal }})</text>
			</text>
		</view>
		<sPullScroll ref="pullScroll" :heightStyle="heightStyle" :pullDown="pullDown" :pullUp="loadData"
			:enablePullDown="true" :enablePullUp="true" :top="topNum" :fixed="true" :bottom="0" finishText="我是有底线的...">
			<view class="cu-list menu-avatar contentList">
				<view @click.stop="chooseFood(food)" :data-target="'move-box-' + idx"
					:class="modalName == 'move-box-' + idx ? 'move-cur' : ''" class="cu-item boxfood"
					v-for="(food, idx) in foodList" :key="idx">
					<view class="smallbox">
						<view class="smallbox-img">
							<image width="100%" height="100%" v-if="!food.imgurl" src="/otherPages/static/img/none.png">
							</image>
						...
						<view class="textbox">
							...
							<view v-if="lockEledata" class="lock-ele">
								...
								<text
									class="second-lock">{{ lockEledata.units ? lockEledata.units.split('/')[0] : '' }}</text>
								)
							</view>
				...
				<view class="content">
					<view class="food-list">
						<view class="list-item">
							...
							<view class="calorie">
								{{ heatNum && heatNum > 0.1 ? heatNum.toFixed(1) : heatNum ? heatNum.toFixed(3) : '' }}千卡
							</view>
						</view>
					</view>
					<view class="calculate">
						<view class="weight">
							...
							<view @click="changeUnit(u, ids)" v-for="(u, ids) in unitList" :key="ids"
								:class="currIndex == ids ? 'active-unit' : ''" class="unit">
					...
					<view class="amount">
						...
						<view class="input-box">
							...
							<input placeholder=" " :disabled="true" :clearable="false" :border="true" maxlength="20"
								v-model="choiceNum" type="number" />
						...
						<view class="smallbox_car">
							...
							<view class="textbox_car">
								...
								<view class="food-utis_car">
									...
									<text class="utis_car">/
										{{ food.unit_weight_g ? food.amount * food.unit_weight_g : food.amount + food.unit }}</text>
								</view>
							</view>
						</view>
						<view class="relian">
					...
					<view class="cu-list menu text-left">
						<view class="cu-item" v-for="(item, index) in unitList" :key="index">
							<label class="flex justify-between align-center flex-sub">
								<view class="flex-sub">{{ item.unit_amount ? item.unit_amount + item.unit : item.unit }}
								</view>
								<radio class="round" :class="radioIndex == index ? 'checked' : ''"
									:checked="radioIndex == index ? true : false" :value="index.toString()"></radio>
							</label>
						</view>
					</view>
				</radio-group>
		...
		<mx-date-picker style="z-index: 1290;" :format="dateFormat" :show="showTimePicker" :type="timeType"
			:value="currentVal" :show-tips="true" :begin-text="'入住'" :end-text="'离店'" :show-seconds="true"
				...
				<view class="filter-box">
					<view class="filter-item-box">
						...
						<view class="child-data">
							<cascader-selector ref="cascader" insert hideButton @clickTag="changeFilter($event, '食材')"
								:srvInfo="dietClassifySrv"></cascader-selector>
						</view>
					</view>
				</view>
				<view class="button-box"><button type="primary" class="cu-btn bg-blue" @click="hideModal">确定</button>
				</view>
			</view>
		</view>
				...
				<view class="filter-box">
					<view class="filtrate-wrap">
						<view class="filtrate-item-wrap">
							<view v-for="(item, index) in menuAgList" :key="index"
								...
								<view class="" style="display: flex;  flex-flow: wrap;">
									<view @click="chooseMenu(item, cate)" v-for="(cate, i) in item.children" :key="i"
										:class="cate.choose ? 'cate-active' : ''" class="filtrate-item-right">
										<text>{{ cate.title }}</text>
										<u-icon size="24"
											v-show="cate.current_num == 1 && item.type !== 'food' && childChooseArrLength == 1"
											name="arrow-downward"></u-icon>
										<u-icon size="24"
											v-show="cate.current_num == 2 && item.type !== 'food' && childChooseArrLength == 1"
											name="arrow-upward"></u-icon>
										<text
											v-show="cate.current_num == 1 && item.type !== 'food' && childChooseArrLength >= 2">(高)</text>
										<text
											v-show="cate.current_num == 2 && item.type !== 'food' && childChooseArrLength >= 2">(中)</text>
										<text
											v-show="cate.current_num == 3 && item.type !== 'food' && childChooseArrLength >= 2">(低)</text>
									</view>
								</view>
							</view>
						</view>
						<view v-if="childChooseArr.length > 0" class="filtrate-choose">
							...
							<view @click="tagClick(item)" v-for="(item, index) in childChooseArr" :key="index"
								class="filtrate-choose-item">
								<text class="cu-tag" :text="item.title" closeable :show="item.choose" type="warning"
									mode="light">{{ item.title }}</text>
		...
		methods: {
			...
			tofeedback() {
				...
				if (!this.isShowMyList) {
					...
					uni.navigateTo({
						url: '/publicPages/newForm/newForm?serviceName=srvhealth_mixed_food_nutrition_contents_add&type=add&addType=onwer&fieldsCond=' +
				...
				if (value >= 0) {
					...
					if (Number(this.choiceNum) && !this.radioLabel) {
						...
						if (this.radioLabel.unit_amount) {
							...
							this.heatNum = Number(this.choiceNum) * ((this.radioLabel.amount / 100) * this.currFood
								.unit_energy);
						}
					}
					if (value == 0) {
			...
			chooseMenu(parent, child) {
				if (child.value === '饭馆') {
					...
					if (parent.type === 'subclass') {} else if (child.value != '不限' && child.value != 'default') {
						child.current_num += 1;
					...
					if (parent.type === 'capacity' || parent.type === 'vitamin' || parent.type === 'mineral' || parent
						.type === 'mingle') {
						...
						if (childChooseArr.length === 1) {
							...
							if (childChooseArr.length == 2 && childChooseArr[1].current_num == 1 && child.value !=
								childChooseArr[0].value) {
						...
						menuData.forEach(par => {
							if (par.type === parent.type) {
								par.children.forEach(p => {
									...
									if (child.value !== '不限' && child.value !== 'default' && child
										.value === p.value && child.current_num == 4) {
										child.current_num = 0;
										p.choose = false;
										par.children[0].choose = true;
									} else if (child.value !== '不限' && child.value !== 'default' && child
										.value === p.value && child.current_num != 0) {
										p.choose = true;
										...
										if ((p.value === '不限' || p.value == 'default') && child.value === p
											.value) {
											p.choose = true;
										} else {
											p.choose = false;
			...
			toFoodsDetail(itemFood) {
				...
				if (this.searchArg.type === 'food') {
					uni.navigateTo({
						url: '/pages/specific/health/foodDetail/foodDetail?foods=' + encodeURIComponent(JSON
							.stringify(itemFood)) + '&filters=' + encodeURIComponent(JSON.stringify(this
							.filterArr))
					});
				} else if (this.searchArg.type === 'sport') {
					uni.navigateTo({
						url: '/pages/specific/health/sportsDetail/sportsDetail?sport=' + encodeURIComponent(JSON
							.stringify(itemFood))
					});
					console.log('点击运动详情');
				}
			...
			async getChooseSportList() {
				...
				if (res.data.state === 'SUCCESS' && res.data.data.length >= 0) {
					...
					for (let i = 0; i < this.chooseFoodArr.length; i++) {
						let urls = self.$api.downloadFile + this.chooseFoodArr[i].image + '&bx_auth_ticket=' + uni
			...
			async getSearchValue(value) {
				...
				let req = {
					...
					colNames: ['*'],
					condition: [{
						colName: this.searchArg.colName,
						ruleType: 'like',
						value: value
					}]
				};
				...
				for (let i = 0; i < resData.length; i++) {
					if (resData[i][self.searchArg.imgCol]) {
						...
						url = self.$api.getFilePath + fileDatas[0].fileurl + '&bx_auth_ticket=' + uni.getStorageSync(
			...
			async getFoodsList(order = null, cond = null, type = null, serviceName = null) {
				let self = this;
				let url = this.getServiceUrl('health', serviceName ? serviceName : this.searchArg.serviceName,
					'select');
				let req = {
				...
				if (data && data.length > 0) {
					...
					for (var i = 0; i < data.length; i++) {
						if (data[i].image) {
							url = self.$api.downloadFile + data[i].image + '&bx_auth_ticket=' + uni.getStorageSync(
								'bx_auth_ticket') + '&thumbnailType=fwsu_100';
			...
			chooseFood(e) {
				...
				if (this.pageType === 'food') {
					...
					if (!this.isShowMyList) {
						...
						uni.navigateTo({
							url: '/otherPages/chooseFood/chooseFood?currFood=' + food + '&type=owner' + this
								.pageDetType
						});
					}
				}
