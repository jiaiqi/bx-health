<template>
	<view class="other-wrap">
			...
			<view v-else-if="type && type === 'sleep'" class="item-wrap">
				...
				<view class="item-list">
					...
					<bx-checkbox-group v-model="inputVal.sleep_event" mode="button">
						<bx-checkbox v-model="item.checked" class="checkbox" color="#2979ff" v-for="(item, i) in sleepEventOption" :key="item.value"
						 :name="item.value">
							{{ item.label }}
						</bx-checkbox>
					</bx-checkbox-group>
				...
				<view class="item-list">
					...
					<bx-checkbox-group v-model="inputVal.wake_up_status" mode="button">
						<bx-checkbox v-model="item.checked" class="checkbox" color="#2979ff" v-for="(item, i) in wakeUpStatusOption" :key="item.value"
						 :name="item.value">
							{{ item.label }}
						</bx-checkbox>
					</bx-checkbox-group>
		...
		<view class="other-bot">
			<button type="primary" class="cu-btn btn bg-cyan shadow-blur" :class="{ disabled: !canSave }" @click="submitRecord"
			 v-if="!isSubmit">保存</button>
			<button class="cu-btn bg-grey btn" @click="back" v-if="isSubmit">返回</button>
