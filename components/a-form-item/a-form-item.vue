    ...
    <view class="form-item-content"
      ...
      <bx-radio-group class="form-item-content_value radio-group" :mode="optionMode" v-model="fieldData.value"
        ...
        <bx-radio :iconSize="fieldData.iconSize" class="radio" color="#2979ff" v-for="item in radioOptions"
          :disabled="fieldData.disabled ? fieldData.disabled : false" :name="item.value" :serial-char="item.serialChar">
          {{ item.label }}
        </bx-radio>
      </bx-radio-group>
      ...
      <view class="form-item-content_value textarea" v-else-if="fieldData.type === 'textarea'">
        <textarea class="textarea-content" :disabled="fieldData.disabled" :adjust-position="true"
          :show-confirm-bar="true" v-model="fieldData.value" :placeholder="fieldData.disabled?'':'开始输入'"></textarea>
      </view>
      ...
      <input type="text" :class="{disabled:fieldData.disabled}" @input="onInput" placeholder="请输入" @blur="onBlur"
      ...
      <view class="cu-dialog" @tap.stop="">
        <view class="tree-selector">
          <view class="content">
            ...
            <bx-checkbox-group v-if="modalName === 'MultiSelector'" class="form-item-content_value checkbox-group"
              v-model="fieldData.value" mode="button">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      addService() {
        return this.optionListV2?.allow_input === '编辑选择' && this.optionListV2?.add_srv_cfg?.srv || ''
      },
      addApp() {
        return this.addSrvCfg?.app || this.optionListV2?.srv_app || uni.getStorageSync('activeApp') || "";
      },
      radioOptions() {
        ...
        } else if (this.selectorData?.length) {
          return this.selectorData.map(item => {
            return {
              ...item,
              label: item[this.field.option_list_v2.key_disp_col],
              value: item[this.field.option_list_v2.refed_col],
            }
          })
        }
      },
      ...
      label_width() {
        ...
        if (['RichText', 'Selector'].includes(this.fieldData.type)) {
          result = '100%';
        }
        return result;
      },
    ...
    methods: {
      ...
      onFormSubmit(data){
        ...
        this.getSelectorData(cond).then(_ => {
          if (this.fieldData.value) {
