<template>
	<view class="date">
		...
		<swiper :style="{height:(!showArrow?1*80: retract ? 2 * 80 : (week_list.length + 1) * 80 ) + 'rpx'}" :current="current"
		 circular @change="change_date">
			<swiper-item>
				...
				<view @click="open" class="retract icon" v-if="showArrow"><text class="iconfont next icon-fanhui" :class="[retract ? '' : 'retract_icon']" /></view>
			</swiper-item>
			<swiper-item>
